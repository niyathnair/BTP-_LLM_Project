...
os.environ["TRANSFORMERS_CACHE"] = "E:/transformers_cache"
...
def setup_llm(llm_model, hf_api, device):
    try:
        bnb_config = BitsAndBytesConfig(
            load_in_8bit=False,
            load_in_4bit=True,
        ...
        llm = HuggingFaceLLM(
            ...
            tokenizer_name=llm_model,
            model_name=llm_model,
            device_map="cuda",  # Set device map based on the available device
            model_kwargs=model_kwargs,
            tokenizer_kwargs=tokenizer_kwargs,
        )
...
def main(args):
    try:
        ...
        print(f"Check availability:{torch.cuda.is_available()}")
        ...
        llm = setup_llm(llm_model, hf_api, device)
        if not llm:
            print("LLM setup failed. Exiting.")
            return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process some integers.")
    parser.add_argument("--data-directory", type=str, default="./data", help="Path to the data directory")
